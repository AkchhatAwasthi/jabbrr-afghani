...
const Checkout = () => {
  ...
  const handleNextStep = () => {
    const minOrderVal = toNumber(settings.min_order_amount) || 300;
    if (subtotal < minOrderVal) {
      toast({
        title: "Minimum Order Not Met",
        description: `Minimum order amount is ${formatCurrency(minOrderVal, settings.currency_symbol)}. Please add more items to your cart.`,
        variant: "destructive",
      });
      return;
    }
  ...
  const updateUserProfile = async () => {
    ...
    try {
      const updates: any = {};
      if (customerInfo.name) updates.full_name = customerInfo.name;
  ...
  const handlePlaceOrder = async () => {
    ...
    const minOrderVal = Number(settings.min_order_amount) || 300;
    if (subtotal < minOrderVal) {
      toast({
        title: "Minimum Order Not Met",
        description: `Minimum order amount is ${settings.currency_symbol}${minOrderVal.toFixed(2)}. Please add more items.`,
        variant: "destructive",
      });
      return;
    }
            ...
            try {
              ...
              if (!currentUser) {
                const guestOrder = {
                  ...
                  total: total,
                  paymentMethod: paymentMethod,
                  paymentStatus: 'paid', // Online is paid immediately
  ...
  if (cartItems.length === 0) {
    return (
      <div className="container mx-auto px-4 py-16 text-center min-h-[60vh] flex flex-col items-center justify-center bg-background">
        <h1 className="text-3xl text-primary uppercase font-inter font-normal tracking-wide mb-4">Your cart is empty</h1>
  ...
  if (settingsLoading) {
    return (
      <div className="container mx-auto px-4 py-16 text-center bg-background min-h-screen">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center space-x-4">
            <Button variant="ghost" onClick={() => navigate(-1)} size="sm" className="text-muted-foreground hover:text-primary hover:bg-white/5">
              <ArrowLeft className="h-4 w-4 mr-2" />
              Back
            </Button>
            <h1 className="text-3xl text-primary uppercase font-inter font-normal tracking-wide">Checkout</h1>
          </div>
          ...
          <div className="xl:col-span-1 order-1 xl:order-2">
            <Card className="sticky top-4 border-[#E6D5B8] bg-[#FFF8F0] shadow-sm">
              <CardHeader className="border-b border-[#E6D5B8]">
                <CardTitle className="text-[#783838] uppercase font-inter font-normal tracking-wide">Order Summary</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4 pt-6">
                ...
                {currentStep < 4 && (
                  <>
                    <div className="space-y-3">
                      <h4 className="font-normal text-sm text-[#5D4037] font-inter uppercase tracking-wide">
                      ...
                      {cartItems.slice(0, 3).map((item) => (
                        <div key={item.id} className="flex items-center space-x-3 py-2">
                          ...
                          <div className="flex-1 min-w-0">
                            <h5 className="font-normal text-sm truncate text-[#783838] font-inter uppercase tracking-wide">{item.name}</h5>
                            <p className="text-xs text-[#5D4037] font-inter">
                              {item.weight} × {item.quantity}
                            </p>
                          </div>
                          <div className="text-sm font-normal text-[#2C1810] font-inter">
                ...
                <div className="flex justify-between font-normal text-lg text-[#2C1810] font-inter">
                ...
                <div className="hidden xl:block">
                  {currentStep < 4 && (
                    <Button
                      className="w-full bg-[#8B2131] hover:bg-[#701a26] text-white font-inter font-normal tracking-widest uppercase"
                      size="lg"
            ...
            {currentStep < 4 ? (
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <div className="text-right mr-4 font-inter">
                    <p className="text-sm text-[#5D4037] uppercase tracking-wide">Total</p>
                    <p className="font-normal text-[#8B2131]">{formatPrice(total)}</p>
                  </div>
                  {currentStep > 1 && (
                    <Button
                      variant="outline"
