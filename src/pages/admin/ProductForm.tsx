...
const ProductForm = ({ product: propProduct, isEdit = false }: ProductFormProps) => {
  ...
  const fetchProduct = async () => {
    ...
    try {
      ...
      const loadedData = data as any;
      setFormData({
        ...loadedData,
        new_arrival: loadedData.marketing_info?.new_arrival || false
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const productData = {
        ...
        // new_arrival is stored in marketing_info since column doesn't exist
        images: images,
      ...
      if (error.code === '23505' || error.message?.includes('products_sku_key')) {
        toast({
          title: "Duplicate SKU",
          description: "This SKU is already in use. Please enter a unique SKU or click 'Generate' to create one.",
          variant: "destructive",
        });
      } else {
        toast({
