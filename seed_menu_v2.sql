...
-- clear existing data
DELETE FROM reviews;
-- assumes order items are json and don't need distinct clearing if checking for foreign keys isn't an issue. 
-- If order_items was a table, we'd clear it. Checking schema showed it's JSON in orders table, so this is fine.
